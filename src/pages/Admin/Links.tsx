...
const Links: React.FC = () => {
    ...
    const handleDeleteLink = () => {
        ...
        if (confirmed) {
            // Make an API call to delete the link from the database
            axios.post('https://studentportal.lcsinhs.com/scripts/survey-delete.php')
                .then((response) => {
                    if (response.data.success) {
                        showToast("Link Successfully Deleted", "success");
                        localStorage.removeItem("surveyLink");
                        setSurveyLink(""); // Update the state to reflect the removal
    ...
    const handleDeleteLink2 = () => {
        const confirmed = window.confirm("Are you sure you want to delete the current request form link?");
        if (confirmed) {
            // Make an API call to delete the link from the database
            axios.post('https://studentportal.lcsinhs.com/scripts/enrolment-delete.php')
                .then((response) => {
                    if (response.data.success) {
                        showToast("Link Successfully Deleted", "success");
                        localStorage.removeItem("enrolLink");
                        setEnrolLink(""); // Update the state to reflect the removal
                        setEditing(true); // Set editing to true
                    } else {
                        showToast("Failed to delete link", "danger");
                    }
            ...
            {isDesktop ?
                <>
                    <IonContent color={'light'}>
                        ...
                        <div className="ttl-margin">
                            <IonLabel className="ttl">Request Form Link</IonLabel>
                        </div>
                        ...
                        <div className="disp-center-flex">
                            {enrolLink && !editingE && (
                                <div className="disp-block">
                                    <IonLabel>Current Link to Request Form:</IonLabel>
                                    <div className="spacer-w-s" />
